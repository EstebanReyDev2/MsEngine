...
export default function NexusShift({ onBack, currentUser, onRefreshUser }: NexusShiftProps) {
  ...
  const generateTrial = useCallback((currentRule: Rule) => {
    ...
    // Ensure one station matches the rule, the other doesn't
    ...
    // Simplified: force logic check in selection
  }, []);
  ...
  }, [packet, stations, activeRule, feedback, streak, playSound, generateTrial, changeRule]);

  useEffect(() => {
    if (gameState !== 'playing') return;
    
    timerRef.current = setTimeout(() => {
        handleDecision('left'); // Auto-timeout treated as wrong
    }, responseWindow);
  ...
  const renderPacket = (p: DataPacket, sizeClass = "w-24 h-24") => (
    <div className={`${sizeClass} flex flex-col items-center justify-center border-4 rounded-xl shadow-lg`} style={{ borderColor: HEX_COLORS[p.color] }}>
      ...
      {p.shape === 'square' && <div className="w-12 h-12" style={{ backgroundColor: HEX_COLORS[p.color] }} />}
      {p.shape === 'circle' && <div className="w-12 h-12 rounded-full" style={{ backgroundColor: HEX_COLORS[p.color] }} />}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200 p-6 flex flex-col items-center justify-center font-mono relative overflow-hidden">
        {/* Context Ring */}
      <motion.div 
      ...
      {gameState === 'intro' && (
        <button onClick={() => { setGameState('playing'); changeRule(); }} className="bg-cyan-600 px-8 py-4 rounded-lg font-bold text-xl uppercase tracking-widest hover:bg-cyan-500">Iniciar Nexus</button>
      )}

      {gameState === 'playing' && (
        <div className="w-full max-w-4xl flex justify-between items-center z-10">
          <div onClick={() => handleDecision('left')} className="cursor-pointer">
            {stations && renderPacket(stations.left)}
            <span className="text-zinc-500 text-xs mt-2 block text-center">[FLECHA IZQ]</span>
          </div>

          <motion.div animate={{ y: [0, -20, 0] }} transition={{ repeat: Infinity, duration: 2 }}>
            {packet && renderPacket(packet, "w-40 h-40")}
          </motion.div>

          <div onClick={() => handleDecision('right')} className="cursor-pointer">
            {stations && renderPacket(stations.right)}
            <span className="text-zinc-500 text-xs mt-2 block text-center">[FLECHA DER]</span>
          </div>
        </div>
      )}

      <div className="absolute top-6 left-6 text-cyan-400 font-bold uppercase text-xs tracking-widest">
        Regla: {activeRule}
      </div>
      <div className="absolute top-6 right-6 text-zinc-500 font-bold uppercase text-xs tracking-widest">
        Puntos: {score}
      </div>
    </div>
  );
